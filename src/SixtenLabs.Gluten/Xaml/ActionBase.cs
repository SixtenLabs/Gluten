	...
	public abstract class ActionBase : DependencyObject
	{
		...
		private void UpdateActionTarget(object oldTarget, object newTarget)
		{
			...
			else
			{
				...
				targetMethodInfo = newTargetType.GetMethod(MethodName);

				if (targetMethodInfo == null)
					logger.Warn("Unable to find method {0} on {1}", MethodName, newTargetType.Name);
				else
		...
		protected internal void AssertTargetSet()
		{
			...
			{
				var ex = new ActionNotFoundException(String.Format("Unable to find method {0} on target {1}", MethodName, Target.GetType().Name));
				logger.Error(ex);
				throw ex;
			}
		}
		...
		{
			logger.Info("Invoking method {0} on target {1} with parameters ({2})", MethodName, Target, parameters == null ? "none" : String.Join(", ", parameters));

			try
			{
				TargetMethodInfo.Invoke(Target, parameters);
			}
			catch (TargetInvocationException e)
			{
				// Be nice and unwrap this for them
				// They want a stack track for their VM method, not us
				logger.Error(e.InnerException, String.Format("Failed to invoke method {0} on target {1} with parameters ({2})", MethodName, Target, parameters == null ? "none" : String.Join(", ", parameters)));
