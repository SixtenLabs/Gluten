...
{
	/// <summary>
	/// Default implementation of ViewManager. Responsible for locating, creating, and settings up Views. 
	/// Also owns the View.Model and View.ActionTarget attached properties
	/// </summary>
	public class ViewManager : IViewManager
	{
		...
		public ViewManager(Func<Type, object> viewFactory, List<Assembly> viewAssemblies)
		{
			...
			ViewAssemblies.AddRange(viewAssemblies);
		}

		/// <summary>
		/// Gets or sets the delegate used to retrieve an instance of a view
		/// </summary>
		public Func<Type, object> ViewFactory { get; }
		...
		{
			return ViewAssemblies.SelectMany(x => x.GetExportedTypes()).FirstOrDefault(t => t.FullName.Equals(viewName));
		}
		...
		protected virtual string ViewTypeNameForModelTypeName(string modelTypeName)
		{
			...
			transformed = Regex.Replace(transformed, string.Format(@"(?<=.){0}(?=s?\.)|{0}$", Regex.Escape(ViewModelNameSuffix)), Regex.Escape(ViewNameSuffix));
		...
		protected virtual Type LocateViewForModel(Type modelType)
		{
			...
			var viewType = ViewTypeForViewName(viewName);
		...
		public virtual UIElement CreateViewForModel(object model)
		{
			...
			var view = (UIElement)ViewFactory(viewType);
